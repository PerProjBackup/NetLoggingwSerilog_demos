...
namespace CoreTodosAPI
{
  public class SwaggerSecurityOperationFilter : IOperationFilter
  {
    ...
    {
      _apiScope = apiScope; this.authorizationOptions = authorizationOptions;
    }

    public void Apply(Operation operation, OperationFilterContext context)
    {
      var requiredClaimsTypes = new List<string> { _apiScope };

      var controllerPolicies = context.ApiDescription.ControllerAttributes(); //TryGetMethodInfo();  //
      var actionPolicies = context.ApiDescription.ActionAttributes();
      var policies = controllerPolicies.Union(actionPolicies).Distinct();

      if (policies.OfType<AllowAnonymousAttribute>().Any()) return;  // must be an anonymous method
