...
namespace CoreFlogger
{
  public static class Flogger
  {
    ...
    static Flogger()
    {
      _perfLogger = new LoggerConfiguration().WriteTo
        .File(path: Environment.GetEnvironmentVariable("LOGFILE_PERF")).CreateLogger();
      _usageLogger = new LoggerConfiguration().WriteTo
        .File(path: Environment.GetEnvironmentVariable("LOGFILE_USAGE")).CreateLogger();
      _errorLogger = new LoggerConfiguration().WriteTo
        .File(path: Environment.GetEnvironmentVariable("LOGFILE_ERROR")).CreateLogger();
      _diagnosticsLogger = new LoggerConfiguration().WriteTo
        .File(path: Environment.GetEnvironmentVariable("LOGFILE_DIAG")).CreateLogger();
    }
