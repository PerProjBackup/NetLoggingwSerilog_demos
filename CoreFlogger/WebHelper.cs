...
namespace CoreFlogger
{
  ...
  {
    public static string Activuty { get; private set; }
    ...
    {
      var details = GetWebFlogDetail(product, layer,
            activityName, context, addtionalInfo);
    ...
    {
      var details = GetWebFlogDetail(product, layer, message,
            context, diagnosticInfo);
      Flogger.WriteDiagnostic(details);
    }

    public static void LogWebError(string product, string layer,
            Exception ex, HttpContext context)
    {
      var details = GetWebFlogDetail(product, layer, null,
            context, null);
    ...
    private static void GetUserData(FlogDetail detail, HttpContext context)
    {
      ...
      if (user != null)
      {
        ...
        foreach (var claim in user.Claims)
        {
          ...
          else if (claim.Type == "name") userName = claim.Value;
